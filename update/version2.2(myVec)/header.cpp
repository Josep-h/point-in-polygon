...
bool CallForPoint1(int id)
{
    ...
    for(;i<sz-1;i++)
    {
        j=i+1;
        //int s=(i+2)%sz;
        xtp1=(*polygon_test_now)[i].first,ytp1=(*polygon_test_now)[i].second;
        xtp2=(*polygon_test_now)[j].first;
        if((xtp1<p.x&&xtp2>p.x)\
        ||(xtp1>p.x&&xtp2<p.x))
        {
            //if(polygon_in[i].first==polygon_in[j].first)
            //continue;
            k=((*polygon_test_now)[j].second-ytp1)/(xtp2-xtp1);
            b=ytp1-xtp1*k;
            y=k*p.x+b;
            if(y>p.y)
            ct++;
    ...
    j=0;
        xtp1=(*polygon_test_now)[i].first,ytp1=(*polygon_test_now)[i].second;
        xtp2=(*polygon_test_now)[0].first;
        if((xtp1<p.x&&xtp2>p.x)\
        ||(xtp1>p.x&&xtp2<p.x))
        {
            k=((*polygon_test_now)[j].second-ytp1)/((*polygon_test_now)[j].first-xtp1);
            b=ytp1-xtp1*k;
            y=k*p.x+b;
            if(y>p.y)
            ct++;
...
void QuaryPoint1(TYPE x,TYPE y,vector<int> &ans)
{
    TYPE mind[2],maxd[2];
    ...
    ans.reserve(mpoly_ans.ct);
    for(int i=0;i!=mpoly_ans.ct;i++)
    ans.push_back(mpoly_ans.ids[i]);
    return;
}
...
void myVec::push(int id)
{
    ids[ct]=id;
    ct++;
}
inline void myVec::resize(int n)
{
    ct=0;
    ids=new int[n];
}