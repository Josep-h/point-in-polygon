...
#include <iostream>
using namespace std;
https://blog.csdn.net/qithon/article/details/50766136
template<typename T>
struct Point{
    T x;
    T y;
    int id;
    Point(){}
    Point(T _x, T _y, int id) :x(_x), y(_y), id(id){}
};
...
public:
    ...
    void RegionResearch(ElemType left, ElemType right, ElemType botom, ElemType top,vector<int> &ans);
    void clear();
private:
    Node<ElemType>* root;
    ...
    void RegionResearch(Node<ElemType>* t, ElemType left, ElemType right, ElemType botom, ElemType top,vector<int> &ans);
...
template<typename T>
void QuardTree<T>::RegionResearch(Node<T>* t, T left, T right, T botom, T top,vector<int> &ans)
{
    if (t == NULL) return;
    T xc = t->pt.x;
    T yc = t->pt.y;
    if (In_Region(t->pt, left, right, botom, top)){ ans.push_back(t->pt.id); }
    if (t->R[0] != NULL && Rectangle_Overlapse_Region(xc, right, yc, top, left, right, botom, top))
    {
        RegionResearch(t->R[0], xc>left?xc:left, right, yc>botom?yc:botom, top,ans);
    }
    if (t->R[1] != NULL && Rectangle_Overlapse_Region(left, xc, yc, top, left, right, botom, top))
    {
        RegionResearch(t->R[1], left, xc>right?right:xc, yc>botom?yc:botom, top,ans);
    }
    if (t->R[2] != NULL && Rectangle_Overlapse_Region(left, xc, botom, yc, left, right, botom, top))
    {
        RegionResearch(t->R[2], left, xc<right?xc:right, botom, yc<top?yc:top,ans);
    }
    if (t->R[3] != NULL && Rectangle_Overlapse_Region(xc, right, botom, yc, left, right, botom, top))
    {
        RegionResearch(t->R[3], xc>left ? xc : left, right, botom, yc<top ? yc : top,ans);
    }
}
...
template<typename T>
void QuardTree<T>::RegionResearch(T left, T right, T botom, T top,vector<int> &ans)
{
    RegionResearch(root, left, right, botom, top,ans);
}
